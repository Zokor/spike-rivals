...
For project-level installation (both Claude and Codex):
```bash
# Project-level skill (recommended for this project)
export WEB_GAME_SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.agents/skills/develop-web-game"
...
Alternative for global Codex installation:
```bash
...
This skill is installed in `.agents/skills/` for use with both Claude and Codex.
...
- `$WEB_GAME_CLIENT` (project path: `.agents/skills/develop-web-game/scripts/web_game_playwright_client.js`) — Playwright-based action loop with virtual-time stepping, screenshot capture, and console error buffering. You must pass an action burst via `--actions-file`, `--actions-json`, or `--click`.

## References

- `$WEB_GAME_ACTIONS` (project path: `.agents/skills/develop-web-game/references/action_payloads.json`) — example action payloads (keyboard + mouse, per-frame capture). Use these to build your burst.
